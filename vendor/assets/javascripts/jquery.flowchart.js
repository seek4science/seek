$j(function() {
    ...
    $j.widget("flowchart.flowchart", {
        ...
        _create: function() {
            ...
            this.objs.layers.operators = $j('<div class="flowchart-operators-layer unselectable"></div>');
            this.objs.layers.operators.appendTo(this.element);

            this.objs.layers.temporaryLink = $j('<svg class="flowchart-temporary-link-layer"></svg>');
        ...
        _initEvents: function() {
            ...
            this.objs.layers.operators.on('click', '.flowchart-operator-connector', function() {
                ...
                if (self.options.canUserEditLinks) {
                    self._connectorClicked($jthis.closest('.flowchart-operator').data('operator_id'), $jthis.data('connector'), $jthis.data('sub_connector'), $jthis.closest('.flowchart-operator-connector-set').data('connector_type'));
                }
            });
        ...
        getConnectorPosition: function(operatorId, connectorId, subConnector) {
            ...
            var $jconnector = operatorData.internal.els.connectorArrows[connectorId][subConnector];
            ...
            var y = (connectorOffset.top - elementOffset.top - 1) / this.positionRatio + parseInt($jconnector.css('border-left-width'));
        ...
        _getOperatorFullElement: function(operatorData) {
            ...
            if (operatorData.properties.shape == "oval") {
                ...
                op_title = '<div class="flowchart-operator-title anti_parallelogram"></div>'
            }
            var $joperator = $j(div);
            ...
            var $joperator_inputs_outputs = $j('<div class="flowchart-operator-inputs-outputs"></div>');

            $joperator_inputs_outputs.appendTo($joperator);

            var $joperator_inputs = $j('<div class="flowchart-operator-inputs"></div>');
            $joperator_inputs.appendTo($joperator_inputs_outputs);

            var $joperator_outputs = $j('<div class="flowchart-operator-outputs"></div>');
            ...
            function addConnector(connectorKey, connectorInfos, $joperator_container, connectorType) {
                var $joperator_connector_set = $j('<div class="flowchart-operator-connector-set"></div>');
            ...
            for (var key_o in infos.outputs) {
                if (infos.outputs.hasOwnProperty(key_o)) {
                    addConnector(key_o, infos.outputs[key_o], $joperator_outputs, 'outputs');
                }
            }
        ...
        _createSubConnector: function(connectorKey, connectorInfos, fullElement) {
            ...
            var $joperator_connector = $j('<div class="flowchart-operator-connector"></div>');
            ...
            var $joperator_connector_label = $j('<div class="flowchart-operator-connector-label"></div>');
            $joperator_connector_label.text(connectorInfos.label.replace('(:i)', subConnector + 1));
            $joperator_connector_label.appendTo($joperator_connector);

            var $joperator_connector_arrow = $j('<div class="flowchart-operator-connector-arrow"></div>');

            $joperator_connector_arrow.appendTo($joperator_connector);

            var $joperator_connector_small_arrow = $j('<div class="flowchart-operator-connector-small-arrow"></div>');
            $joperator_connector_small_arrow.appendTo($joperator_connector);

            fullElement.connectors[connectorKey].push($joperator_connector);
            fullElement.connectorArrows[connectorKey].push($joperator_connector_arrow);
            fullElement.connectorSmallArrows[connectorKey].push($joperator_connector_small_arrow);
        },
            ...
            if (this.options.canUserMoveOperators) {
                ...
                fullElement.operator.draggable({
                    ...
                    start: function(e, ui) {
                        ...
                        pointerX = (e.pageX - elementOffset.left) / self.positionRatio - parseInt($j(e.target).css('left'));
                        pointerY = (e.pageY - elementOffset.top) / self.positionRatio - parseInt($j(e.target).css('top'));
                    },
                    drag: function(e, ui) {
                        if (self.options.grid) {
                            ...
                            if (!operatorData.internal.properties.uncontained) {
                                var $jthis = $j(this);
                                ui.position.left = Math.min(Math.max(ui.position.left, 0), self.element.width() - $jthis.outerWidth());
                                ui.position.top = Math.min(Math.max(ui.position.top, 0), self.element.height() - $jthis.outerHeight());
                            }
