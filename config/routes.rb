ActionController::Routing::Routes.draw do |map|
  ...
  map.resources :data_files, :collection=>{:test_asset_url=>:post, :preview_permissions => :get},:member => {:download => :get,:plot=>:get, :data => :get,:preview_publish=>:get,:publish=>:post, :request_resource=>:post, :update_tags_ajax=>:post, :explore=>:get},:new=>{:upload_for_tool => :post}  do |data_file|
