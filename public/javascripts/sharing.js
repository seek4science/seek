...
function init_sharing() {
  PRIVATE = parseInt($('const_private').value);
	...
	replaceFavouriteGroupRedboxActionURL('edit_existing_f_group_redbox');
}
...
function test() {
	alert('the test');
	var redbox_id_prefix = 'hidden_content_edit_existing_f_group_redbox';
	
  var x=$$('div');
  for (var i = 0; i < x.length; i++)
  	if (x[i].id.substr(0, redbox_id_prefix.length) == redbox_id_prefix) {
			alert(x[i].id);
			alert(x[i].outerHTML);
			alert(x[i].nextSibling.outerHTML);
			alert(x[i].ancestors()[0].innerHTML);
  	}
}
...
function replaceFavouriteGroupRedboxActionURL(redbox_id_part) {
  var redbox_id_prefix = 'hidden_content_' + redbox_id_part;
  var search_str = 'parameters:\'';
  
  var parameters = {};
	parameters['id'] = $('favourite_group_select').options[$('favourite_group_select').selectedIndex].value;
	
  
  // given that ID of the redbox DIV is unique, can find it and modify the URL in the relevant place 
  var x=$$('div');
  for(var i = 0; i < x.length; i++)
    if(x[i].id.substr(0, redbox_id_prefix.length) == redbox_id_prefix) {
      var main_ancestor_element = x[i].ancestors()[0];
			var link_to_redbox_element_html = main_ancestor_element.innerHTML;
      
      //alert(link_to_redbox_element_html);
      
      var first_str_index_after_url = link_to_redbox_element_html.indexOf(search_str) + (search_str.length - 1); // -1 to make it point to where this insertion to be made
      
			var authenticity_token_start_index = link_to_redbox_element_html.indexOf('encodeURIComponent(\'', first_str_index_after_url) + 20;
			parameters['authenticity_token'] = link_to_redbox_element_html.substr(authenticity_token_start_index,40);
			
			var str_index_to_close_param_hash = link_to_redbox_element_html.indexOf('}', authenticity_token_start_index) + 1;
			
      //alert(favourite_group_id_param);
      //alert(link_to_redbox_element_html.substr(first_str_index_after_url));
			
			alert(link_to_redbox_element_html.substring(0,first_str_index_after_url));
			alert(link_to_redbox_element_html.substring(first_str_index_after_url, str_index_to_close_param_hash));
			alert(link_to_redbox_element_html.substr(str_index_to_close_param_hash));
			
      var str_t = link_to_redbox_element_html.substring(0,first_str_index_after_url) + 
			            Object.toJSON(parameters).replace(/\"/g, '\'') +
									//link_to_redbox_element_html.substring(first_str_index_after_url, str_index_to_close_param_hash) +
									'}' +
									link_to_redbox_element_html.substr(str_index_to_close_param_hash); 
		  
			alert(str_t);
			//
			//link_to_redbox_element_html = str_t;
      
			
			alert(main_ancestor_element.innerHTML);
			
			
			main_ancestor_element.innerHTML = str_t; 
			//$(main_ancestor_element).update(str_t);
