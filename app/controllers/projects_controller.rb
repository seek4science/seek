...
class ProjectsController < ApplicationController
  ...
  def asset_report
    ...
    @types.each do |type|
      ...
      @public_assets[type] = @project.send(type.table_name).all_authorized_for(action, nil)
      ...
      projects_shared = user.nil? ? [] : @project.send(type.table_name).all_authorized_for('download', user)
