class PostsController < ApplicationController
  before_filter :find_post,      except: %i[index create monitored search]
  before_filter :login_required, except: %i[index monitored search show]
  ...
  def index
    conditions = []
    %i[user_id forum_id topic_id].each { |attr| conditions << Post.send(:sanitize_sql, ["#{Post.table_name}.#{attr} = ?", params[attr]]) if params[attr] }
