...
module ISAHelper
  ...
  def delete_ISA_icon model_item, user
    ...
    if model_item.can_delete?(user)
      return "<li>"+image_tag_for_key('destroy',url_for(model_item),"Delete #{item_name}", {:style => "display:block;width:90%;", :confirm=>"Are you sure?",:method=>:delete },"Delete #{item_name}") + "</li>"
    elsif !model_item.can_delete?(user) && model_item.can_edit?(user)
      explanation=unable_to_delete_text model_item
      return "<li><span style='display:block;width:90%;' class='disabled_icon disabled' onclick='javascript:alert(\"#{explanation}\")' title='#{tooltip_title_attrib(explanation)}' >"+image('destroy', {:alt=>"Delete",:class=>"disabled"}) + " Delete #{item_name} </span></li>"
    end
  end
