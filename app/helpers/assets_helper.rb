module AssetsHelper
  ...
  def mini_file_delete_icon(fileinfo, user)
    ...
    if fileinfo.can_delete?(user)
      html = content_tag(:div) { image_tag_for_key('destroy', polymorphic_path([fileinfo.asset, fileinfo], method: :delete, code: params[:code]), "Delete #{item_name}", { data: { confirm: 'It cannot be undone. Are you sure?' }, method: :delete }, "Delete (cannot be reverted)") }
