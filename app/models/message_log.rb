...
class MessageLog < ApplicationRecord  
  ...
  scope :activation_email_logs, -> (person) {where(message_type: ACTIVATION_EMAIL,subject:person).order(created_at: :asc)}
  ...
  scope :recent_project_membership_requests, ->(person,project) { where(subject:project).where(sender: person).project_membership_requests.recent }
  ...
  def self.log_project_membership_request(sender, project, institution, comments)
    ...
    MessageLog.create(subject: project, sender: sender, details: details.to_json, message_type: PROJECT_MEMBERSHIP_REQUEST)
