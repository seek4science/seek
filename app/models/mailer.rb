class Mailer < ActionMailer::Base
  ...
  def feedback(user, topic, details, send_anonymously)
    ...
    mail(from: Seek::Config.noreply_sender,
         to: admin_emails,
         subject: "#{Seek::Config.project_name} Feedback provided - #{topic}",
         reply_to: reply_to)
  end
  ...
  def file_uploaded(uploader, receiver, file)
    ...
    mail(from: Seek::Config.noreply_sender,
         ...
         subject: "#{Seek::Config.project_name} - File Upload",
         reply_to: uploader.person.email_with_name)
  end

  def request_publishing(owner, publisher, resources)
    @owner = owner
    publish_notification owner, publisher, resources, "A #{Seek::Config.project_name} member requests you make some items public"
  end

  def request_publish_approval(gatekeeper, publisher, resources)
    @gatekeeper = gatekeeper
    publish_notification gatekeeper, publisher, resources, "A #{Seek::Config.project_name} member requested your approval to publish some items."
  end
  ...
  def request_contact(user, resource, details)
    ...
    mail(from: Seek::Config.noreply_sender,
         ...
         subject: "A #{Seek::Config.project_name} member requests to discuss with you regarding #{resource.title}")
  end
  ...
  def activation_request(user)
    ...
    mail(from: Seek::Config.noreply_sender,
         ...
         subject: "#{Seek::Config.project_name} account activation")
  end
  ...
  def forgot_password(user)
    ...
    mail(from: Seek::Config.noreply_sender,
         to: user.person.email_with_name,
         subject: "#{Seek::Config.project_name} - Password reset")
  end
  ...
  def welcome(user)
    ...
    mail(from: Seek::Config.noreply_sender,
         to: user.person.email_with_name,
         subject: "Welcome to #{Seek::Config.project_name}")
  end
  ...
  def contact_admin_new_user(user)
    ...
    mail(from: Seek::Config.noreply_sender,
         to: admin_emails,
         reply_to: user.person.email_with_name,
         subject: "#{Seek::Config.project_name} member signed up")
  end

  def project_changed(project)
    @project = project
    recipients = admin_emails | @project.project_administrators.collect{|m| m.email_with_name}
    subject = "The #{Seek::Config.project_name} #{t('project')} #{@project.title} information has been changed"
  ...
  def announcement_notification(site_announcement, notifiee_info)
    ...
    mail(from: Seek::Config.noreply_sender,
         ...
         subject: "#{Seek::Config.project_name} Announcement: #{site_announcement.title}")
  end
  ...
  def notify_user_projects_assigned(person,new_projects)
    ...
    mail(from: Seek::Config.noreply_sender,
         ...
         subject: "You have been assigned to a #{Seek::Config.project_name} project")
  end
  ...
  def programme_activation_required(programme, creator)
    ...
    mail(from: Seek::Config.noreply_sender,
         to: admin_emails,
         subject: "The #{Seek::Config.project_name} #{t('programme')} #{programme.title} was created and needs activating"
    )
  end
  ...
  def programme_activated(programme)
    ...
    mail(from: Seek::Config.noreply_sender,
         ...
         subject: "The #{Seek::Config.project_name} #{t('programme')} #{programme.title} has been activated"
    )
  end
  ...
  def programme_rejected(programme, reason)
    ...
    mail(from: Seek::Config.noreply_sender,
         to: programme.programme_administrators.map(&:email_with_name),
         subject: "The #{Seek::Config.project_name} #{t('programme')} #{programme.title} has been rejected"
    )
  end
  ...
  def gatekeeper_response(response, requester, gatekeeper, items_and_comments)
    ...
    mail(from: Seek::Config.noreply_sender,
         to: requester.email_with_name,
         subject: "A #{Seek::Config.project_name} #{I18n.t('asset_gatekeeper').downcase} #{response} your publishing requests.",
