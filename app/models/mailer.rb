class Mailer < ActionMailer::Base
  ...
  def feedback user,topic,details,send_anonymously,base_host
    subject "#{Seek::ApplicationConfiguration.get_application_name} Feedback provided - #{topic}"
    recipients admin_emails
    from Seek::ApplicationConfiguration.get_noreply_sender
  ...
  def request_resource(user,resource,details,base_host)
    ...
    subject "A #{Seek::ApplicationConfiguration.get_application_name} member requested a protected file: #{resource.title}"
  ...
  def signup(user,base_host)
    subject     "#{Seek::ApplicationConfiguration.get_application_name} account activation"
  ...
  def forgot_password(user,base_host)
    subject    "#{Seek::ApplicationConfiguration.get_application_name} - Password reset"
    recipients user.person.email_with_name
    from       Seek::ApplicationConfiguration.get_noreply_sender
    sent_on    Time.now
  ...
  def welcome(user,base_host)
    subject    "Welcome to #{Seek::ApplicationConfiguration.get_application_name}"
    recipients user.person.email_with_name
    from       Seek::ApplicationConfiguration.get_noreply_sender
    sent_on    Time.now
  ...
  def contact_admin_new_user_no_profile(details,user,base_host)
    ...
    subject    "#{Seek::ApplicationConfiguration.get_application_name} member signed up"
    recipients admin_emails
    from       Seek::ApplicationConfiguration.get_noreply_sender
    reply_to   user.person.email_with_name
    sent_on    Time.now
  ...
  def announcement_notification(site_announcement, notifiee_info,base_host)
    subject "#{Seek::ApplicationConfiguration.get_application_name} Announcement: #{site_announcement.title}"
