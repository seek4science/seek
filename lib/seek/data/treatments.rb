    ...
    class Treatments
      ...
      def initialize(xml = nil)
        ...
        unless xml.nil?
          ...
          extract_from_document doc unless doc.nil?
        end
      end
      ...
      def extract_as_table(treatments_heading_row, sample_col, treatments_first_col, treatments_last_col, sheet)
        table = []
        sheet_name = sheet.attributes['name']
        cells = sheet.find("//ss:sheet[@name='#{sheet_name}']/ss:rows/ss:row/ss:cell[@row >= '#{(treatments_heading_row)}' and (@column = '#{sample_col}' or (@column>='#{treatments_first_col}' and @column<='#{treatments_last_col}'))]")
        ...
        table = table[0...lasti]
        table.each do |row|
          row.fill('', row.size, maxwidth - row.size) if row.size < maxwidth
          row.map! { |v| v.nil? ? '' : v }
        end
