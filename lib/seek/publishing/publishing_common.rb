  ...
  module Publishing
    module PublishingCommon
      ...
      def is_gatekeeper_approval_required?(object)
        params[:policy_attributes] && params[:policy_attributes][:access_type].to_i > Policy::NO_ACCESS && object.gatekeeper_required? && !User.current_user.person.is_asset_gatekeeper_of?(object)
      end
