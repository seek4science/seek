    ...
    def handle_download(disposition = 'attachment', image_size = nil)
      if @content_blob.url.blank?
        if @content_blob.file_exists?
          ...
          redirect_on_error @asset_version, "Unable to find a copy of the file for download, or an alternative location. Please contact an administrator of #{Seek::Config.project_name}."
        end
      else
        begin
