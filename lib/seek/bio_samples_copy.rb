...
module Seek
  ...
  class BioSamples
    ...
    def populate_assay sheet, filename
      ...
      @study = Study.find_by_title(study_title).detect{|s|s.can_edit? User.current_user}
      unless @study
    ...
    def populate_specimen sheet, specimen_name_cell
      ...
      unless specimen
        ...
        unless specimen.organism == organism &&
            ...
            now = Time.now
            new_sp.title = "#{speicmen_title}-#{now}"
    ...
    def populate_sample sheet, sample_name_cell
      ...
      unless sample
        ...
        unless sample.specimen == @specimens[row] &&
            sample.sample_type == sample_type &&
            sample.tissue_and_cell_type == tissue_and_cell_type &&
            sample.donation_date == donation_date &&
            sample.institution == institution &&
            sample.comments == comments
            sleep(1);
