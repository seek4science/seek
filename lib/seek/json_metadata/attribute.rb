module Seek
  module JSONMetadata
    module Attribute
      ...
      included do
        self.belongs_to :sample_attribute_type, inverse_of: self.table_name.to_sym
        self.belongs_to :sample_controlled_vocab, inverse_of: self.table_name.to_sym

        self.validates :sample_attribute_type, presence: true

        # validates that the attribute type is CV if vocab is set, and vice-versa
        self.validate :sample_controlled_vocab_and_attribute_type_consistency

        # validates that the attribute type is SeekSample if linked_sample_type is set, and vice-versa
        self.validate :linked_sample_type_and_attribute_type_consistency

        self.delegate :controlled_vocab?, :seek_sample?, :seek_strain?, :seek_resource?, to: :sample_attribute_type, allow_nil: true
      end
