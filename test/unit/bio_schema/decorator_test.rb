...
class DecoratorTest < ActiveSupport::TestCase
  ...
  test 'CreativeWork' do
    ...
    assert_equal identifier, decorator.identifier
    assert_equal 'yellow, lorry', decorator.keywords
    ...
    assert_equal [{ :@type => 'Project', :@id => "http://localhost:3000/projects/#{project.id}", :name => project.title }], decorator.provider
