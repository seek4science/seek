...
class DecoratorTest < ActiveSupport::TestCase
  ...
  test 'CreativeWork' do
    ...
    properties = decorator.attributes.collect(&:property).collect(&:to_s).sort
    assert_equal ['@id', 'creator', 'dateCreated', 'dateModified', 'description', 'encodingFormat', 'image', 'isPartOf', 'keywords', 'license', 'name', 'producer', 'subjectOf', 'url', 'version'], properties
  end
