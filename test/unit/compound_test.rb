...
class CompoundTest < ActiveSupport::TestCase
  ...
  test "should create the association compound has_many mappings, through mapping_links table" do
    ...
    assert mapping1.save!
    assert mapping2.save!
    assert compound.save!
    assert mapping_link1.save!
    assert mapping_link2.save!
  ...
  test 'should create the compound has_many studied_factors, through studied_factor_links table ' do
    User.with_current_user  users(:aaron) do
      ...
      assert fs2.save!
      assert compound.save!
      assert studied_factor_link1.save!
      assert studied_factor_link2.save!
  ...
  test 'should create the compound has_many experimental_conditions, through experimental_condition_links table ' do
    User.with_current_user  users(:aaron) do
      ...
      assert ec2.save!
      assert compound.save!
      assert experimental_condition_link1.save!
      assert experimental_condition_link2.save!
