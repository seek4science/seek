...
class UtilTest < ActiveSupport::TestCase
  ...
  test 'searchable types' do
    ...
    expected = [Assay, DataFile, Document, Event, Institution, Investigation, Model, Node, Person, Presentation, Programme, Project, Publication, Sample, SampleType, Sop, Strain, Study, Workflow]
    ...
    with_config_value :events_enabled, false do
      ...
      types = Seek::Util.searchable_types
      expected = [Assay, DataFile, Document, Institution, Investigation, Model, Node, Person, Presentation, Programme, Project, Publication, Sample, SampleType, Sop, Strain, Study, Workflow]
      assert_equal expected.map(&:to_s).sort, types.map(&:to_s).sort
    end
    ...
    with_config_value :programmes_enabled, false do
      Seek::Util.clear_cached
      types = Seek::Util.searchable_types
      expected = [Assay, DataFile, Document, Event, Institution, Investigation, Model, Node, Person, Presentation, Project, Publication, Sample, SampleType, Sop, Strain, Study, Workflow]
      assert_equal expected.map(&:to_s).sort, types.map(&:to_s).sort
    end
  end
