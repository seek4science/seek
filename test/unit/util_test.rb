...
class UtilTest < ActiveSupport::TestCase
  test 'creatable types' do
    ...
    expected = [DataFile, Model, Presentation, Publication, Sop, Workflow, Assay, Investigation, Study, Event,  Strain]
  ...
  test 'searchable types' do
    ...
    expected = [Assay, DataFile, Event, Institution, Investigation, Model, Person, Presentation, Programme, Project, Publication, Sop, Strain, Study, Workflow]
    ...
    with_config_value :events_enabled,false do
      ...
      types = Seek::Util.searchable_types
      expected = [Assay, DataFile, Institution, Investigation, Model, Person, Presentation, Programme, Project, Publication, Sop, Strain, Study, Workflow]
      assert_equal expected.map { |t| t.to_s }, types.map { |t| t.to_s }
    end
    ...
    with_config_value :programmes_enabled,false do
      Seek::Util.clear_cached
      types = Seek::Util.searchable_types
      expected = [Assay, DataFile, Event, Institution, Investigation, Model, Person, Presentation, Project, Publication, Sop, Strain, Study, Workflow]
      assert_equal expected.map { |t| t.to_s }, types.map { |t| t.to_s }
    end
