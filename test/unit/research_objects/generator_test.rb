...
class GeneratorTest < ActiveSupport::TestCase
  ...
  test 'generate for investigation with duplicate files' do
    ...
    assert_iuncludes paths, "models/#{model.ro_package_path_id_fragment}/xxx.txt"
    assert_iuncludes paths, "models/#{model.ro_package_path_id_fragment}/1-xxx.txt"
    assert_iuncludes paths, "models/#{model.ro_package_path_id_fragment}/2-xxx.txt"
  end
  ...
  def check_contents(file, inv)
    ...
    inv.studies.each do |study|
      assert_iuncludes paths, "#{study.research_object_package_path}metadata.rdf"
      assert_iuncludes paths, "#{study.research_object_package_path}metadata.json"
      study.assays.each do |assay|
        assert_iuncludes paths, "#{assay.research_object_package_path}metadata.rdf"
        assert_iuncludes paths, "#{assay.research_object_package_path}metadata.json"
      end
    end
    assets = inv.assets
    assert_equal 7, assets.count
    assets.each do |asset|
      assert_iuncludes paths, "#{asset.research_object_package_path}metadata.json"
      assert_iuncludes paths, "#{asset.research_object_package_path}metadata.json"
    end

    # simple check for assets contents, using model with image to check the image is there
    assert_iuncludes paths, "models/#{@assay_asset5.asset.ro_package_path_id_fragment}/cronwright.xml"
    assert_iuncludes paths, "models/#{@assay_asset5.asset.ro_package_path_id_fragment}/file_picture.png"

    # and a model with 2 files
    assert_iuncludes paths, "models/#{@assay_asset6.asset.ro_package_path_id_fragment}/cronwright.xml"
    assert_iuncludes paths, "models/#{@assay_asset6.asset.ro_package_path_id_fragment}/rightfield.xls"

    # and finally the RO specific files
    assert_iuncludes paths, 'mimetype'
    assert_iuncludes paths, '.ro/manifest.json'
  end
