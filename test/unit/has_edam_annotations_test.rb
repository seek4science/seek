...
class HasEdamAnnotationsTest < ActiveSupport::TestCase
  ...
  test 'edam vocab present' do
    refute @workflow.send(:edam_topics_vocab)
    refute @workflow.send(:edam_operations_vocab)

    topics_vocab = Factory(:edam_topics_controlled_vocab)
    operations_vocab = Factory(:edam_operations_controlled_vocab)

    assert_equal topics_vocab, @workflow.send(:edam_topics_vocab)
    assert_equal operations_vocab, @workflow.send(:edam_operations_vocab)
  end
end
