...
class CVAttributeTypeHandlerTest < ActiveSupport::TestCase
  ...
  test 'test value' do
    ...
    handler = Seek::Samples::AttributeTypeHandlers::CVAttributeHandler.new(attr)
  ...
  test 'exception thrown for missing controlled vocab' do
    ...
    handler = Seek::Samples::AttributeTypeHandlers::CVAttributeHandler.new(attr)
    assert_raises(Seek::Samples::AttributeTypeHandlers::CVAttributeHandler::MissingControlledVocabularyException) do
      assert handler.validate_value?('Granny Smith')
    end
  end
