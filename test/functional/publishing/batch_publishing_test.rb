...
class BatchPublishingTest < ActionController::TestCase
  ...
  test "get batch_publishing_preview" do
    ...
    assert_select "div.type_and_title", :count=>total_asset_count do
      publish_immediately_assets.each do |a|
      ...
      assert_select "div.type_and_title img[src*=?][title=?]",/lock.png/, /Private/, :count => total_asset_count
    end

    assert_select "div.secondary", :text => /Publish/, :count => total_asset_count do
      publish_immediately_assets.each do |a|
