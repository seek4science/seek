...
class SinglePublishingTest < ActionController::TestCase
  ...
  test "sending publishing request when doing publish for can_not_publish item" do
    ...
    assert df.can_manage?,"The data file must be manageable for this test to succeed"
    assert ResourcePublishLog.last_waiting_approval_log(df).nil?,"The publishing request for this data file must not be sent for this test to succeed"
    ...
    assert !ResourcePublishLog.last_waiting_approval_log(df).nil?,"The publishing request for this data file should be sent after requesting"
  end
